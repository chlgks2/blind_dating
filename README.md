...
<img src="ai/docs/images/after_anime2d.png" width="260" alt="anime2d 스타일 변환 결과">

`anime2d` 스타일 변환 결과. 입력 원본은 개인 사진이라 저장소에 포함하지 않았습니다.
...
클라이언트의 "결제 성공" 신호를 신뢰하지 않고, 서버가 PortOne 토큰을 발급받아 `api.iamport.kr/payments/{imp_uid}`로 실제 상태를 재조회합니다. `imp_uid`에 `unique` 제약 + `get_or_create`로 중복 반영을 막고, 양측이 모두 결제해야 `is_chat_open=True`가 됩니다. 이 플래그는 **REST와 WebSocket 양쪽에서 재검증**됩니다.
